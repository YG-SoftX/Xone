#!/usr/bin/env bash
# YG DocX — Backup Script (cPanel compatible)
# Cron: 0 2 * * * bash /path/to/yg-docx/scripts/backup.sh >> /path/to/logs/backup.log 2>&1
set -euo pipefail
...
mkdir -p "$BACKUP_DIR"
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting YG DocX backup..."

...
<head>
    ...
    <title>@yield('title', 'Sovereign Dialogue') — YGXONE</title>
...
<body class="antialiased selection:bg-teal-500/30">
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="hidden md:flex flex-col w-72 premium-glass border-r border-white/5 z-50">
            ...
            <!-- New Chat/Space Button -->
            <div class="px-6 mb-6">
                <button onclick="document.getElementById('new-space-modal').classList.remove('hidden')" class="w-full py-4 bg-gradient-to-r from-teal-600 to-blue-600 text-white font-black rounded-2xl hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-teal-600/20 uppercase tracking-widest text-xs">
                    <i class="fas fa-plus mr-2"></i> {{ __("New Dialogue") }}
                </button>
            </div>

            <nav class="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
                <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest px-4 mb-4">{{ __("Communication Nodes") }}</p>
                
                <a href="{{ route('chat.index') }}" class="flex items-center px-4 py-4 rounded-2xl text-gray-400 font-bold transition-all sidebar-item sidebar-item-active">
                    <i class="fas fa-hashtag w-8 text-lg"></i>
                    <span class="text-sm tracking-tight">{{ __("Sovereign Spaces") }}</span>
                </a>

                <a href="#" class="flex items-center px-4 py-4 rounded-2xl text-gray-400 font-bold transition-all sidebar-item">
                    <i class="fas fa-user-group w-8 text-lg"></i>
                    <span class="text-sm tracking-tight">{{ __("Direct Signals") }}</span>
                </a>

                <div class="pt-8">
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest px-4 mb-4">{{ __("Ecosystem Bridge") }}</p>
                    <a href="https://ai.ygxone.com" class="flex items-center px-4 py-4 rounded-2xl text-gray-400 font-bold transition-all sidebar-item">
                        <i class="fas fa-brain w-8 text-lg text-teal-400"></i>
            ...
            <div class="p-6 border-t border-white/5 bg-black/20 mt-auto">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[10px] text-teal-400 font-bold uppercase tracking-widest">{{ __("Signal Active") }}</p>
                    </div>
                </div>
            </div>
        </aside>
            ...
            <header class="h-20 premium-glass border-b border-white/5 flex items-center justify-between px-8 z-40">
                <div class="flex items-center flex-1 space-x-8">
                    <div class="relative w-full max-w-xl">
                        ...
                        <input type="text" placeholder="Search secure dialogues..." class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-6 text-sm text-white placeholder-gray-600 focus:border-teal-500/50 outline-none transition-all">
                    </div>
                ...
                <div class="flex items-center space-x-6 ml-8">
                    <div class="h-10 px-4 bg-white/5 border border-white/10 rounded-xl flex items-center gap-3">
                        ...
                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Signal Encrypted</span>
                    </div>
                </div>
            </header>

            <!-- Workspace Content area -->
            <main class="flex-1 overflow-hidden relative flex flex-col bg-black/20">
                ...
                <footer class="p-6 border-t border-white/5 bg-black/40">
                    <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                        ...
                        <p class="text-[10px] font-black text-gray-700 uppercase tracking-[0.3em]">
                            &copy; {{ date('Y') }} {{ __("YGXONE EMPIRE. SIGNAL SECURED.") }}
                        </p>
                    </div>
                </footer>
            </main>

#!/usr/bin/env bash
# YG Master — Backup Script (cPanel compatible)
# Cron: 0 1 * * * bash /path/to/yg-master/scripts/backup.sh >> /path/to/logs/backup.log 2>&1
set -euo pipefail
...
mkdir -p "$BACKUP_DIR"
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting YG Master backup (extended retention: ${KEEP_DAYS}d)..."
...
if [[ -n "${DB_DATABASE:-}" && -n "${DB_USERNAME:-}" ]]; then
    ...
    MYSQL_PWD="${DB_PASSWORD:-}" mysqldump --host="${DB_HOST:-localhost}" --user="$DB_USERNAME" \
        --single-transaction --routines --triggers --no-tablespaces "$DB_DATABASE" | gzip -9 > "$DB_FILE"
    chmod 600 "$DB_FILE"
